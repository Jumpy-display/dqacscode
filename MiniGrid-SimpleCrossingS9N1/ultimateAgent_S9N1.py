...
class DiffQASLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.locked_idx is not None:
            ...
            circuit_out = qnode(x_in, self.circuit_params)
...
def ppo_update(agent, optimizer, memory, next_value=None, next_done=None, batch_size=512, gamma=0.99, gae_lambda=0.95, 
    ...
    for epoch in range(n_epochs):
        for i_batch, batch in enumerate(batches):
            ...
            loss_arch = lambda_arch * -torch.sum(probs * torch.log(probs + 1e-10))  # encourage higher entropy
...
def make_env(seed: int, idx: int):
    def _init():
        ...
        env = gym.make("MiniGrid-SimpleCrossingS9N1-v0", render_mode=None)
