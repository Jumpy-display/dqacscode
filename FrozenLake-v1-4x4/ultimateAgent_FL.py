...
class DiffQASLayer(nn.Module):
    def __init__(self, n_qubits=4, n_layers=2):
        ...
        # Use lightning.qubit with OpenMP (uses available cores)
        try:
        ...
        # --- PRUNED SEARCH SPACE (12 Combinations) ---
        ...
        self.k_active = 2           # start with 2, later set to 1
        self.k_active_start = min(6, self.n_candidates)
        self.k_active_mid = 2
        self.k_active_final = 1
        self.k_stage1_updates = 10    # use k_active_start for updates < this
        self.k_stage2_updates = 100   # use k_active_mid for updates < this; otherwise k_active_final
        self.lock_threshold = 0.95  # lock when max prob >= this
        self.lock_patience_updates = 3   # consecutive PPO updates (not minibatches)
        self.lock_warmup_updates = 20    # don't allow locking before this many updates
        self.lock_check_every = 1         # check locking every N PPO updates
        # Backward-compat alias (older code may reference lock_patience)
    ...
    def create_qnode(self, arch_idx):
        """Creates the QNode for a specific architecture index."""
    ...
    def forward(self, x):
        ...
        probs = torch.softmax(self.struct_weights, dim=0)  # keep grad to struct_weights
...
def one_hot(states: np.ndarray, n_states: int) -> np.ndarray:
...
def train():
    env_name = "FrozenLake-v1_4x4_nonslippery"
    ...
    assert isinstance(dummy.observation_space, gym.spaces.Discrete), "FrozenLake obs space should be Discrete."
    n_states = int(dummy.observation_space.n)
    STACK_K = 4
    input_dim = n_states * STACK_K
    action_dim = int(dummy.action_space.n)
    dummy.close()
    
    def init_stack(onehot: np.ndarray, k: int) -> np.ndarray:
        return np.repeat(onehot[:, None, :], k, axis=1)
    
    def update_stack(stack: np.ndarray, onehot_new: np.ndarray, dones: np.ndarray) -> np.ndarray:
        stack = np.roll(stack, shift=-1, axis=1)
        stack[:, -1, :] = onehot_new
        if np.any(dones):
            stack[dones, :, :] = np.repeat(onehot_new[dones][:, None, :], stack.shape[1], axis=1)
        return stack
    ...
    obs, _ = envs.reset()
    onehot0 = one_hot(obs, n_states)
    obs_stack = init_stack(onehot0, STACK_K)
    next_obs = obs_stack.reshape(NUM_ENVS, -1)
    ...
    for update in range(1, MAX_UPDATES + 1):
        ...
        for step in range(STEPS_PER_UPDATE):
            ...
            train_rewards = rewards.astype(np.float32).copy()
            ...
            if np.any(timeouts):
                ...
                if final_obs_list is not None:
                    ...
                    if len(valid) > 0:
                        final_states = np.asarray([final_obs_list[i] for i in valid], dtype=np.int64)
                        final_onehot = one_hot(final_states, n_states)
                        final_stack = np.repeat(final_onehot[:, None, :], STACK_K, axis=1).reshape(len(valid), -1)
                        final_flat_t = torch.tensor(final_stack, dtype=torch.float32)

                        with torch.no_grad():
                            _, _, _, final_v = agent.get_action_and_value(final_flat_t)
                            final_v = final_v.squeeze(-1).cpu().numpy()
                        train_rewards[valid] += gamma * final_v
            ...
            onehot_new = one_hot(obs, n_states)
            obs_stack = update_stack(obs_stack, onehot_new, dones)
            next_obs = obs_stack.reshape(NUM_ENVS, -1)
        ...
        if last_terminations is None:
            last_terminations = np.zeros(NUM_ENVS, dtype=bool)
        next_done = torch.from_numpy(last_terminations).bool()
        ...
        if ep_count > 0 and ep_ret_mean >= 0.95:
            good_streak += 1
        else:
            good_streak = 0
    ...
    fin_path = os.path.join(ckpt_dir, "final_model.pth")
    torch.save({
        'model_state_dict': agent.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
        'arch_weights': agent.q_layer.struct_weights,
    }, fin_path)
    print(f"Model saved to: {fin_path}")
